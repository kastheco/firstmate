...
config/brief-skill-chain  free-text local skill chain injected verbatim into every generated ship brief; LOCAL, gitignored; absent or whitespace-only = no such section, ship briefs unchanged; no schema, no fixed skill name; inherited by secondmate homes (docs/configuration.md "Local skill chain for generated ship briefs")
