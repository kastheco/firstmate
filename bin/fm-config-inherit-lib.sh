...
# "off" preferences propagate as files. Primary config/brief-skill-chain carries
# the same free-text local skill chain into every ship brief a secondmate's own
# crewmates generate; an absent primary file mirrors as absent downstream, same
# as the other plain items. Primary
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context brief-skill-chain orca-lane-root}"
